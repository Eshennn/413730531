...
function setup() {
  ...
  frameRate(10) //進入draw()函數，每秒執行10次
}
...
function draw() {//開始畫圖，每秒進入draw()會執行60次
 ...
 let r = map(sin(frameCount),-1,1,50,255) //對應，sin的值介於-1到1之間的位置，對應到50~255的位置
 let g = map(cos(frameCount/2),-1,1,50,255) //對應
 let b = map(sin(frameCount/4),-1,1,50,255) //對應
  stroke(r,g,b)
 rotate(sin(angle-i*2)*5)//三角函數sin的值介於-1~1間，*40之後，旋轉角度介於-40~40度
